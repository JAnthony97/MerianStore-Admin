...
TimePicker.prototype.loadContent = function () {
    ...
    html += '</div></div><div class="form-actions align-center"><a class="button btn-small btn-icon text-color-white background-color01 ic-cancel-white cancel">Cancelar</a><a class="button btn-small btn-icon text-color-white background-color02 ic-ok-white ok">Aceptar</a></div></div></div>';
    
    $('body').append(html);

    var spacing = $('#' + this.id + ' .numbers label:first-child').innerHeight() / 2;
    var radius = $('#' + this.id + ' .clock').innerHeight() / 2 - spacing;
    var angle = 360.0 / ($('#' + this.id + ' .numbers label').length - 1);
