function Validator(fields, errorClass) {
    ...
    for (var i = 0; i < this.fields.length; i++) {
        ...
        var minValue = this.fields[i].minValue;
        ...
        if ((type == 'D' || type == 'I') && maxValue !== null) {
            ...
            $(selector).keypress({ maxValue: maxValue }, function (event) {
                if (parseFloat($(this).val() + String.fromCharCode(event.which)) > event.data.maxValue)
                    event.preventDefault();
            });
            $(selector).off('blur');
            $(selector).blur({ maxValue: maxValue }, function (event) {
                if (parseFloat($(this).val()) > event.data.maxValue)
                    $(this).val(event.data.maxValue);
            });
        }

        if ((type == 'D' || type == 'I') && minValue !== null) {
            $(selector).off('keypress');
            $(selector).keypress({ minValue: minValue }, function (event) {
                if (parseFloat($(this).val() + String.fromCharCode(event.which)) < event.data.minValue)
                    event.preventDefault();
            });
            $(selector).off('blur');
            $(selector).blur({ minValue: minValue }, function (event) {
                if (parseFloat($(this).val()) < event.data.minValue)
                    $(this).val(event.data.minValue);
            });
        }
    }
}
...
Validator.prototype.validate = function () {
    ...
    for (var i = 0; i < this.fields.length; i++) {
        ...
        $(selector).each(function (index, item) {
            ...
            var minValue = validator.fields[i].minValue;
            if (typeof minValue === 'undefined')
                minValue = null;
            ...
            if ((type == 'D' || type == 'I') && minValue !== null)
                if (!messageRequired && parseFloat($(item).val()) < minValue)
                    message += 'El valor mínimo para el campo <span class="bold">' + name + '</span> es <span class="bold">' + minValue + '</span>.\n';
